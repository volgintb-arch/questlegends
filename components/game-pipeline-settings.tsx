...
export function GamePipelineSettings({ franchiseeId: propFranchiseeId, onUpdate, onPipelineCreated }: GamePipelineSettingsProps) {
  ...
  const createPipeline = async () => {
    ...
    try {
      const res = await fetch("/api/game-pipelines", {
        method: "POST",
  ...
  const deletePipeline = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/game-pipelines/${id}`, {
        method: "DELETE",
  ...
  const addStage = async () => {
    ...
    try {
      const res = await fetch(`/api/game-pipelines/${selectedPipeline.id}/stages`, {
        method: "POST",
        headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
  ...
  const deleteStage = async (stageId: string) => {
    ...
    try {
      const res = await fetch(`/api/game-pipelines/${selectedPipeline.id}/stages/${stageId}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      })
      if (res.ok) {
  ...
  const updateStageOrder = async (stages: Stage[]) => {
    ...
    try {
      await fetch(`/api/game-pipelines/${selectedPipeline.id}/stages`, {
        method: "PATCH",
        headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
  ...
  const updateStageName = async (stageId: string, name: string) => {
    ...
    try {
      await fetch(`/api/game-pipelines/${selectedPipeline.id}/stages/${stageId}`, {
        method: "PATCH",
        headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-medium">Воронки игр</h2>
          ...
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-3 pt-2">
              ...
              <p className="text-[10px] text-muted-foreground">
                Этапы "Согласовано", "Завершено" и "Отказ" будут добавлены автоматически
              </p>
            ...
            <div className="flex items-center gap-2">
              ...
              <Input
                ...
                onBlur={async (e) => {
                  ...
                  try {
                    const res = await fetch(`/api/game-pipelines/${selectedPipeline.id}`, {
                      method: "PATCH",
                      headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
                ...
                disabled={selectedPipeline.name === "Архив"}
                className="h-7 text-xs flex-1"
              />
              {!selectedPipeline.isDefault && selectedPipeline.name !== "Архив" && (
                <Button
                  variant="ghost"
                  size="sm"
          ...
          <CardContent className="p-3 pt-0 space-y-2">
            <div className="space-y-1">
              {(selectedPipeline.stages || []).map((stage, index) => (
                ...
                >
                  {!stage.isFixed && <GripVertical className="h-3 w-3 text-muted-foreground cursor-grab" />}
                  ...
                  {stage.isFixed && (
                    <span className="text-[9px] px-1 py-0.5 rounded bg-muted text-muted-foreground">
                      {stage.stageType === "completed" ? "Успех" : stage.stageType === "scheduled" ? "График" : "Отказ"}
                    </span>
                  )}
                  {!stage.isFixed && (
                    <>
