...
import { Download, Filter, TrendingUp, CheckCircle2, XCircle, Clock, RefreshCw } from "lucide-react"
...
export function MarketingLeadsReport() {
  ...
  const totals = useMemo(() => {
    const t = { total: 0, confirmed: 0, inProgress: 0, cancelled: 0, revenue: 0 }
  ...
  const exportExcel = () => {
    ...
    const summaryRows = Object.entries(summary)
      ...
      .map(([src, s]) => ({
        ...
        "Конверсия %": s.total > 0 ? ((s.confirmed / s.total) * 100).toFixed(1) + "%" : "0%",
        "Выручка ₽": s.revenue,
      }))
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
        <div className="bg-card border border-border rounded-lg p-3">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-muted-foreground">Всего лидов</span>
          </div>
        ...
        <div className="bg-card border border-border rounded-lg p-3">
          ...
          <p className="text-xl font-bold text-green-500">{totals.confirmed}</p>
        </div>
        <div className="bg-card border border-border rounded-lg p-3">
          <div className="flex items-center gap-2 mb-1">
        ...
        <div className="bg-card border border-border rounded-lg p-3 col-span-2 sm:col-span-1">
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5">
            Конверсия: {totals.total > 0 ? ((totals.confirmed / totals.total) * 100).toFixed(1) : 0}%
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted/30">
                <tr>
                  ...
                  <th className="text-right px-3 py-2 font-medium text-green-500">Подтверждено</th>
                ...
                {Object.entries(summary)
                  ...
                  .map(([src, s]) => (
                    <tr key={src} className="border-t border-border">
                      ...
                      <td className="px-3 py-2 text-right text-green-500">{s.confirmed}</td>
                      ...
                      <td className="px-3 py-2 text-right">
                        {s.total > 0 ? ((s.confirmed / s.total) * 100).toFixed(1) : 0}%
                      </td>
